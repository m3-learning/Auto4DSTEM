...
def mock_data():
    ...
    with h5py.File("mock_py4dstem.h5", "w") as f:
        f.create_dataset(
            "strain_map_root/strain_map/data", data=np.random.rand(4, 256, 256)
        )
...
def test_visualize_simulate_result_init(mock_data, tmp_path):
    ...
    viz_sim_res = VisualizeSimulation(
        rotation=rotation,
    ...
    assert isinstance(viz_sim_res, VisualizeSimulation)
    ...
    assert viz_sim_res.label_rotation.shape == (256, 256), (
        "Label rotation shape mismatch."
    )
