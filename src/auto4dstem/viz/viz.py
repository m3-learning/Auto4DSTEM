...
#    fig, ax = plt.subplots(4, 4, figsize=(22, 20))
    ...
    if add_title:
        labelfigs(
            ax[0, 0], string_add=r"py4d:$\epsilon_{xx}$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[1, 0], string_add=r"$\epsilon_{yy}$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[2, 0], string_add="$\epsilon_{xy}$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[3, 0], string_add=r"$rot$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[0, 2], string_add=r"auto:$\epsilon_{xx}$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[1, 2], string_add=r"$\epsilon_{yy}$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[2, 2], string_add=r"$\epsilon_{xy}$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
        labelfigs(
            ax[3, 2], string_add=r"$rot$", loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
    # save figure
    if save_figure:
...
#        ax[row, col + 1].title.set_text("MAE: " + format(mae_, ".4f"))
        labelfigs(
            ax[row, col + 1], string_add="MAE: " + format(mae_, ".4f"), loc="ct", inset_fraction=(0.05, 0.05),
            style = label_style
        )
    # add label to figure
    remove_all_ticks(ax[0,0],ax[1,0],ax[2,0],ax[3,0],ax[0,2],ax[1,2],ax[2,2],ax[3,2])
