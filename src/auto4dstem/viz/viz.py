...
def Strain_Compare(
    ...
    **kwargs
    ...
    fig,ax = subfigures(4, 4, gaps=(0.6, 0.15))
    ax = np.array(ax).reshape(4, 4)
#    fig, ax = plt.subplots(4, 4, figsize=(22, 20))
    ...
    remove_all_ticks(ax[0,0],ax[1,0],ax[2,0],ax[3,0],ax[0,2],ax[1,2],ax[2,2],ax[3,2])

    # set title of each label
    if add_title:
        labelfigs(
            ax[0, 0], string_add=r"Py4dSTEM", loc="ct", inset_fraction=(inset_fraction[0],0),
            style = label_style
        )
        labelfigs(
            ax[0, 2], string_add=r"Auto4DSTEM", loc="ct", inset_fraction=(inset_fraction[0],0),
            style = label_style
        )
        labelfigs(
            ax[0, 0], string_add=r"$\epsilon_{xx}$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[1, 0], string_add=r"$\epsilon_{yy}$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[2, 0], string_add="$\epsilon_{xy}$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[3, 0], string_add=r"$rot$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[0, 2], string_add=r"$\epsilon_{xx}$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[1, 2], string_add=r"$\epsilon_{yy}$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[2, 2], string_add=r"$\epsilon_{xy}$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
        labelfigs(
            ax[3, 2], string_add=r"$rot$", loc="br", inset_fraction=inset_fraction,
            style = label_style
        )
    # save figure
    if save_figure:
        filtered_kwargs = filter_kwargs(Printer, kwargs)
        printer = Printer(basepath=folder_name, **filtered_kwargs)
        printer.savefig(
            fig, f"/Strain_Map_{title_}", label_figs=ax.ravel(), **kwargs
        )
...
#        ax[row, col + 1].title.set_text("MAE: " + format(mae_, ".4f"))
        labelfigs(
            ax[row, col + 1], string_add="MAE: " + format(mae_, ".4f"), loc="ct", inset_fraction=(inset_fraction[0],0),
            style = label_style
        )
    # add label to figure
    remove_all_ticks(ax[0,0],ax[1,0],ax[2,0],ax[3,0],ax[0,2],ax[1,2],ax[2,2],ax[3,2])
    ...
    if save_figure:
        ...
        printer = Printer(basepath=folder_name, **filtered_kwargs)
        printer.savefig(
            fig, f"/Performance_Comparison_{noise_format}Percent_BKG", label_figs=ax.ravel(), **kwargs
        )
