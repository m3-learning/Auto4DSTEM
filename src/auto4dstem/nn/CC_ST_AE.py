import numpy as np
from ..VizUtil.util import mask_function, center_of_mass
import torch
...
def crop_small_square(center_coordinates,
    ...
    x_coordinate  = (int(center_coordinates[0]-radius),int(center_coordinates[0]+radius))
    
    y_coordinate = (int(center_coordinates[1]-radius),int(center_coordinates[1]+radius))
...
def revise_size_on_affine_gpu(image, 
    ...
    for j, mask in enumerate(mask_list):
        ...
        for i in range(batch_size):
            ...
            x_coordinate ,y_coordinate = crop_small_square(center_coordinates=center.clone(),radius = radius)
            
            small_image = img[i].squeeze()[x_coordinate [0]:x_coordinate [1],y_coordinate[0]:y_coordinate[1]].unsqueeze(0).unsqueeze(1).clone().to(device)
            ...
            if adj_para == None:
                ...
                img[i,:,x_coordinate [0]:x_coordinate [1],y_coordinate[0]:y_coordinate[1]] = re_aff_small_image.squeeze()
...
def make_model_fn(device,
    ...
    encoder = Encoder(en_original_step_size,
                     pool_list,
                     conv_size,
                     device,
                     scale,
